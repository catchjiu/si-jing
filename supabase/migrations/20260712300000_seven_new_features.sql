-- Seven features: start task, video submissions, streak milestones,
-- queen directives, journal, mood status, wishlist fulfillment

-- 1. Task start timestamp
ALTER TABLE public.tasks
  ADD COLUMN IF NOT EXISTS started_at TIMESTAMPTZ;

-- 2. Wishlist fulfillment tracking
...
-- 3. Queen-initiated requests (directives / questions)
ALTER TABLE public.requests
  ADD COLUMN IF NOT EXISTS direction TEXT NOT NULL DEFAULT 'petition'
...
-- Wishlist: slave can update status (seen); queen update policy exists in prior migration
CREATE POLICY "Slave can mark wishlist seen"
