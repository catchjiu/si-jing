...
export function JournalSlideshow({ images, className }: JournalSlideshowProps) {
  ...
  useEffect(() => {
    if (!playing || pausedByHover || count <= 1) return;
    const id = window.setInterval(() => go(1), AUTO_MS);
    return () => window.clearInterval(id);
  }, [playing, pausedByHover, count, go]);
          ...
          {count > 1 && (
            <>
              <button
                type="button"
                onClick={() => go(-1)}
                className="absolute left-2 top-1/2 -translate-y-1/2 rounded-full bg-void/70 p-1.5 text-ivory hover:bg-void/90"
              ...
              <button
                type="button"
                onClick={() => go(1)}
                className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full bg-void/70 p-1.5 text-ivory hover:bg-void/90"
                aria-label="Next photo"
              >
                <ChevronRight className="h-5 w-5" />
              </button>

              <div className="absolute bottom-2 left-1/2 flex -translate-x-1/2 items-center gap-2 rounded-full bg-void/75 px-2.5 py-1">
                <Button
                  type="button"
                  size="sm"
                  variant="ghost"
