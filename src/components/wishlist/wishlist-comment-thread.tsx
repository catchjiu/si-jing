...
export function WishlistCommentThread({
  ...
  const [messages, setMessages] = useState<WishlistMessage[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    const rows = (data as WishlistMessage[]) ?? [];
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`wishlist-messages:${wishlistId}`)
      .on(
        "postgres_changes",
        {
          event: "*",
          schema: "public",
          table: "wishlist_messages",
  ...
  const removeComment = async (message: WishlistMessage) => {
    ...
    try {
      ...
      if (message.image_path) {
        try {
          await removeObject({ bucket: "wishlist", path: message.image_path });
        } catch {
    ...
    try {
      ...
      if (file) {
        ...
        imagePath = await presignAndUpload({
          ...
          ext,
          relativePath: `${profile.id}/comments/${wishlistId}/${Date.now()}.${ext}`,
        });
      }
    ...
    <div className={cn("space-y-4 border-t border-gold/10 pt-4", className)}>
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          placeholder={
            isSlave
              ? "Ask about this item or leave a note…"
              : "Reply on this wishlist item…"
          }
      ...
      <VoiceNotes
        ...
        title={
          isSlave
            ? "Voice comment"
            : `Voice on ${wishlistTitle || "wishlist item"}`
        }
      />
    </div>
  );
