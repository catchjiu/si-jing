...
import { taskNewHref, type TaskLane } from "@/lib/task-lane";

export function TasksBoard({ lane }: { lane: TaskLane }) {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="font-heading text-3xl text-ivory">
            {isSwitch ? (isQueen ? "Daddy's tasks" : "Duties") : "Tasks"}
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {isSwitch
              ? "For slut — kept apart from Queen and slave"
        ...
        <div className="flex gap-2">
          ...
          {isQueen && (
            <Button asChild className="bg-gold text-void hover:bg-gold-muted">
              <Link href={taskNewHref(lane)}>
                <Plus className="mr-2 h-4 w-4" />
                {isSwitch ? "Assign to slut" : "Assign Task"}
              </Link>
            </Button>
          )}
        </div>
